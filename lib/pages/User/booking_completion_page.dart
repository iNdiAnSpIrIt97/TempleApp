...
import 'package:temple_app/config.dart';
...
class _BookingCompletionPageState extends State<BookingCompletionPage> {
  ...
  Future<void> _checkGuestUser() async {
    ...
    setState(() {
      _guestUser = isGuest ?? true; // Default to guest if not set
    });
      ...
      try {
        ...
        if (userDoc.exists) {
          ...
          setState(() {
            ...
            if (_bookingFor == 'Myself' && _guestDetails.isNotEmpty) {
  ...
  void _initializeGuestFields() {
    _guestDetails = [
      GuestDetails(
          initialName: _bookingFor == 'Myself' ? _nameController.text : null)
    ];
  }
  ...
  Future<void> _submitBooking(
      {bool paymentStatus = false, String? paymentId}) async {
    ...
    if (_guestDetails.length < widget.room.occupancy) {
      bool? proceed = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          title: Text('Warning'),
          content: Text(
            'You have chosen a room for ${widget.room.occupancy} but only entered ${_guestDetails.length} guest(s). Booking will be done for ${_guestDetails.length}. Are you sure you want to continue?',
          ),
          actions: [
            TextButton(
    ...
    try {
      ...
      developer.log('Booking created with ID: ${bookingRef.id}');

      // Save to user's room_booking subcollection if not a guest
      if (!_guestUser && _userId != null) {
        ...
        developer.log(
            'Saved to user\'s room_booking subcollection with ID: ${bookingRef.id}');
      }
      ...
      developer.log('Room availability updated successfully');
  ...
  void _handlePaymentSuccess(PaymentSuccessResponse response) {
    ...
    _submitBooking(paymentStatus: true, paymentId: response.paymentId);
  }
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
                      ...
                      leading: Radio<String>(
                        ...
                        onChanged: (value) {
                          setState(() {
                            _bookingFor = value!;
                            _guestDetails = [
                              GuestDetails(initialName: _nameController.text)
                            ];
                          });
                        },
                      ),
              ...
              SizedBox(height: 20),
              if (widget.room.occupancy > 1 || _bookingFor == 'Others') ...[
                ...
                onPressed: _isSubmitting
                    ...
                    : () {
                        if (_formKey.currentState!.validate()) {
                          if (_paymentOption == 'Pay Now') {
                            _initiatePayment();
                          } else {
                            _submitBooking(paymentStatus: false);
                          }
                        }
                      },
